...
import type { AssistantMessage, SystemMessage, ToolResultMessage } from "@/types/chat";
...
function mergeSessionMessages(sessionId: string, messages: Array<MessageRow>): void {
...
const getSession = vi.fn(
  async (id: string): Promise<SessionData | undefined> => state.sessions.get(id),
);
const getSessionMessages = vi.fn(
  async (sessionId: string): Promise<Array<MessageRow>> =>
...
function getPersistedSystemRows(): Array<SystemMessage & Pick<MessageRow, "sessionId" | "status">> {
  return putSessionAndMessages.mock.calls.flatMap(([_session, messages]) =>
    messages.filter(
      (message): message is SystemMessage & Pick<MessageRow, "sessionId" | "status"> =>
        message.role === "system",
    ),
  );
}
...
describe("AgentHost persistence", () => {
  beforeEach(() => {
    ...
    getSessionMessages.mockImplementation(
      async (sessionId: string) => state.messagesBySession.get(sessionId) ?? [],
    );
    ...
    expect(
      putSessionAndMessages.mock.calls.some(([_session, messages]) =>
        messages.some(
          (message) =>
            message.role === "system" &&
            message.fingerprint === "unknown:Runtime stopped before clearing the streaming state.",
        ),
      ),
    ).toBe(false);
  ...
  it("ignores a duplicate terminal snapshot after the turn already finalized", async () => {
    const { AgentTurnPersistence } = await import("@/agent/agent-turn-persistence");
  ...
  it("dedupes repeated system notices for the same classified error", async () => {
    ...
    expect(
      putSessionAndMessages.mock.calls.filter(([_session, messages]) =>
        messages.some((message) => message.role === "system" && message.kind === "github_auth"),
      ),
    ).toHaveLength(1);
  ...
  it("drops orphan tool results while repairing an interrupted turn", async () => {
    ...
    const host = new AgentHost(createSession(), state.messagesBySession.get("session-1") ?? []);
    ...
    expect(
      state.messagesBySession.get("session-1")?.some((message) => message.role === "toolResult"),
    ).toBe(false);
  ...
  it("keeps matching tool results while repairing an interrupted turn", async () => {
    ...
    expect(state.messagesBySession.get("session-1")).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          role: "toolResult",
          toolCallId: "call-1",
  ...
  it("appends multiple system notices for distinct runtime failures", async () => {
    ...
    promptMock
      .mockRejectedValueOnce(new GitHubFsError("EACCES", "Authentication required", "/"))
    ...
    expect(systemKinds).toEqual(expect.arrayContaining(["github_auth", "github_rate_limit"]));
